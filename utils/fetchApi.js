...
export const getdataApiClub = async (url, auth, page, permission) => {
  //QTV
  if (permission === "631c254a7a3a837ce2c22995") {
    const res = await axios.get(
      `${URL}/api/62e0b3885271e2560e8bb7d3/${url}?access_token=${auth.token}&limit=1000&q={"thu_ky": "${auth.profile.email}"}`
    );
    return res;
  }
  //partner
  if (permission === "631c254a7a3a837ce2c229a7") {
    const res = await axios.get(
      `${URL}/api/62e0b3885271e2560e8bb7d3/${url}?access_token=${auth.token}&limit=1000&q={"partner": "${auth.profile.email}"}`
    );
    return res;
  }
  ...
  if (permission === "631c254a7a3a837ce2c229ac") {
    const res = await axios.get(
      `${URL}/api/62e0b3885271e2560e8bb7d3/${url}?access_token=${auth.token}&limit=1000&q={"ds_thanh_vien":{"$elemMatch":{"ma_kh":"${auth.profile.email}"}}}`
    );
    return res;
  }
};
